...
public interface BusinessMemberRepository extends JpaRepository<Member, String> {
	...
	@Query("SELECT m FROM Member m WHERE m.businessNumber IS NOT NULL AND m.businessVerified = :businessVerified AND (" +
		       "  (:searchType = 'email' AND m.email LIKE %:keyword%) OR " +
		       "  (:searchType = 'companyName' AND m.companyName LIKE %:keyword%) OR " +
		       "  (:searchType = 'businessNumber' AND m.businessNumber LIKE %:keyword%) OR " +
		       "  ((:searchType = 'all' OR :searchType IS NULL OR :searchType = '') AND " +
		...
		Page<Member> searchByConditionFilter(@Param("searchType") String searchType, 
		                               @Param("keyword") String keyword, 
		                               @Param("businessVerified") Boolean businessVerified,
		                               Pageable pageable);
	
	@Query("SELECT m FROM Member m WHERE m.businessNumber IS NOT NULL AND m.businessVerified = :businessVerified")
	Page<Member> findAllBusinessFilter(Pageable pageable, @Param("businessVerified") Boolean businessVerified);
